...
public class TickRefreshSupplier<T> implements Supplier<T> {
    ...
    private final Function<T, T> delegate;
    ...
    public TickRefreshSupplier(IPlatform platform, Supplier<T> delegate) {
        ...
        this.delegate = ignored -> delegate.get();
    }

    public TickRefreshSupplier(IPlatform platform, Function<T, T> refresher, T initialValue) {
        this.platform = platform;
        this.delegate = refresher;
    ...
    public T get() {
        ...
        if (this.lastTick != currentTick) {
            synchronized (this) {
                if (this.lastTick != currentTick) {
                    T value = this.delegate.apply(this.value);
                    this.value = value;
